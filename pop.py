...
def _f_identifier():
    """Retrieves a disc identifier from the DJMAX site by clean disc name."""
    def identifier(disc):
        # DJMAX returns up to 20 identifiers at a time (each page lists 20 discs
        # except the last page which has less than 20), the cache dictionary
        # saves these identifers to expedite future lookups.  however, since it
        # is unknown how long these identifiers stay valid, the cache size has
        # been limited to about 25% of the total number of available discs.
        # when the cache exceeds this limit, it will clear itself and rebuild.
        if disc not in cache:
            ...
            cache.update({_clean(record["DISCNAME"]): record["DISCID"] for record in data})
        return cache[disc]
